...
public class NextCommand extends Command {
    ...
    public NextCommand(Corddit corddit) {
        super("next");
            ...
            clearOrContinue(channel, paginators.containsKey(link)).thenRunAsync(() -> {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    channel.sendMessage(embed.build()).queue(message -> {
                        new VoteListener<>(corddit, message, submission, user -> {
                            var member = Objects.requireNonNull(channel.getGuild().getMember(user.getDiscordUser()));
                            corddit.getCommentManager().openCommentThread(submission, channel, user).thenAccept(channelOptional -> {
